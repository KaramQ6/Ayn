...
import { getActiveForests } from '../data/forests.js';
...
export async function updateFireRisk(db, broadcast) {
  ...
  for (const forest of activeForests) {
    try {
      ...
      if (results.length % 10 === 0) {
        console.log(`  🌡️ Progress: ${results.length}/${activeForests.length} forests updated...`);
      }
  ...
  console.log(`🌡️ Updated fire risk for ${results.length}/${activeForests.length} regions`);
  return results;
}
...
function insertDemoRiskData(db, broadcast) {
  ...
  const demoForests = getActiveForests().filter((_, i) => i % 3 === 0 || i < 8); // All Jordan + every 3rd active forest
