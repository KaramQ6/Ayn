...
import { TrendingUp, PieChart as PieIcon, BarChart3, X, Loader2 } from 'lucide-react';
...
function AnalyticsPanel({ isOpen, onClose }) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="!max-w-[95vw] sm:!max-w-[95vw] lg:!max-w-6xl w-full bg-[#0a140e] border-white/10 rounded-[24px] p-0 overflow-hidden outline-none h-[90vh] flex flex-col">
        {/* Header */}
        <DialogHeader className="sticky top-0 z-10 bg-[#0a140e]/95 backdrop-blur-xl border-b border-white/5 p-5 flex-row justify-between items-center bg-black/20 space-y-0">
          <div className="flex items-center gap-3">
            <TrendingUp className="w-5 h-5 text-green-500" />
            <DialogTitle className="text-lg font-bold text-white/90">{t('analyticsPanel.title')}</DialogTitle>
        ...
        <DialogDescription className="sr-only">
          Detailed analytics and trends for forest monitoring including fire hotspots, risk distribution, and reporting patterns.
        </DialogDescription>

        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center p-12">
            <Loader2 className="w-8 h-8 text-emerald-500 animate-spin mb-4" />
            <p className="text-white/50 text-sm font-data uppercase tracking-widest">{t('analyticsPanel.loading')}</p>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto custom-scrollbar p-6 grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
            {/* Fire Trend */}
            ...
            {/* Risk Distribution */}
            <div className="bg-black/30 border border-white/5 rounded-2xl p-5 flex flex-col col-span-1 md:col-span-2 lg:col-span-1">
              <div className="flex items-center gap-2 mb-4">
            ...
            <div className="bg-black/30 border border-white/5 rounded-2xl p-5 flex flex-col col-span-1 md:col-span-2 lg:col-span-1">
              ...
              {rainRiskSeries.length === 0 ? (
                <div className="min-h-[250px] lg:min-h-[350px] flex items-center justify-center text-white/20 text-xs font-data">{t('analyticsPanel.noRainData')}</div>
              ) : (
                <div className="flex-1 min-h-[250px] lg:min-h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
            ...
            {/* Reports by Type */}
            <div className="bg-black/30 border border-white/5 rounded-2xl p-5 flex flex-col col-span-1 md:col-span-2">
              <div className="flex items-center gap-2 mb-4">
                <PieIcon className="w-4 h-4 text-blue-400" />
                <h3 className="text-xs font-data uppercase tracking-widest text-white/60">{t('analyticsPanel.reportsByType')}</h3>
              </div>
              {pieData.length === 0 ? (
                <div className="min-h-[250px] lg:min-h-[350px] flex items-center justify-center text-white/20 text-xs font-data">{t('analyticsPanel.noReports')}</div>
              ) : (
                <div className="flex-1 min-h-[250px] lg:min-h-[350px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={pieData} cx="50%" cy="50%" innerRadius={80} outerRadius={120} paddingAngle={3} dataKey="value" label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
                        {pieData.map((_, i) => (
