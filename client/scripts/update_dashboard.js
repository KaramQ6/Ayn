...
const targetPath = path.join(__dirname, '../src/Dashboard.jsx');
let content = fs.readFileSync(targetPath, 'utf8');

const newMainBlock = `      {/* MAIN DASHBOARD */}
      <main className="flex-1 p-4 md:p-6 flex flex-col lg:flex-row gap-6 overflow-hidden max-w-[1800px] mx-auto w-full h-[calc(100vh-80px)]">
        ...
        <div className="w-full lg:w-[400px] flex-shrink-0 flex flex-col gap-6 overflow-y-auto custom-scrollbar lg:pe-2">
          ...
          <div className="flex-1 bg-[#050A07] border border-white/10 rounded-[24px] flex flex-col min-h-[200px] overflow-hidden shadow-[inset_0_4px_24px_rgba(0,0,0,0.5)] border-t-2 border-t-green-500/30">
        ...
        <div className="flex-1 relative bg-[var(--canopy-green)] border border-white/10 rounded-[28px] overflow-hidden shadow-2xl min-h-[500px] flex flex-col">
          ...
          <div className="absolute top-6 start-6 z-[500] bg-[#0A140E]/80 backdrop-blur-xl border border-white/10 px-4 py-2.5 rounded-2xl flex items-center gap-3 shadow-lg">
            ...
            <span className="text-xs font-data text-white/90 uppercase tracking-widest">{t('dashboard.map.firmsTelemetry', 'SATELLITE TELEMETRY')}</span>
            <div className="h-4 w-px bg-white/10 mx-1"></div>
            <span className="text-[10px] font-data text-[#10B981]/80">{t('dashboard.map.clickToReport', 'CLICK MAP TO REPORT')}</span>
          </div>
          ...
          <div className="absolute bottom-6 end-6 z-[500] w-[320px] pointer-events-auto hidden md:flex flex-col">
            <div className="bg-[#0A140E]/90 backdrop-blur-2xl border border-white/10 rounded-[24px] shadow-[0_8px_32px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden max-h-[300px]">
              <div className="p-4 border-b border-white/5 flex items-center gap-2 bg-[#1A211D]/50 w-full">
                ...
                <h3 className="text-xs font-bold uppercase tracking-widest text-white/90">{t('dashboard.risk.title', 'REGIONAL RISK INDEX')}</h3>
                <Shield className="w-3 h-3 text-emerald-500/50 ms-auto" />
              </div>
              <div className="p-4 overflow-y-auto flex-1 space-y-4 custom-scrollbar">
                {risks.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-white/20 py-4">
                    <span className="text-[10px] font-data uppercase tracking-widest">{t('dashboard.risk.awaiting', 'AWAITING DATA')}</span>
                  </div>
...
const startIndex = content.indexOf('      {/* MAIN DASHBOARD */}');
const endIndex = content.indexOf('      </main>') + '      </main>'.length;

if (startIndex !== -1 && endIndex !== -1) {
  content = content.substring(0, startIndex) + newMainBlock + content.substring(endIndex);
  fs.writeFileSync(targetPath, content, 'utf8');
  console.log('Successfully updated Dashboard.jsx layout');
} else {
  console.error('Failed to locate existing MAIN DASHBOARD block');
}
